...
function loadNotifications(){
	var shell, notifPath, fso, f, f1, fc, s;
	
	// Get the "Notifications" folder where each individual xml file will exist
	shell  = new ActiveXObject( "WScript.Shell" );
	notifPath = shell.ExpandEnvironmentStrings( unescape( "%ALLUSERSPROFILE%\\CDP\\SnapBack\\Notifications" ) );
	fso = new ActiveXObject("Scripting.FileSystemObject");
	f = fso.GetFolder(notifPath);
	...
	if (myCount == 0){
		...
		div.innerHTML = "<div class='notifications-none'>You have no new notifications.</div>";
	}
